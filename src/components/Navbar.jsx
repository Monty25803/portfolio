  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${
        scrolled || open ? "nav-blur border-b border-[var(--color-border)]" : "border-b border-transparent"
      }`}
    >
      <nav className="section-shell flex items-center justify-between py-4">
        <a href="#home" className="font-serif text-xl font-semibold tracking-tight" onClick={() => setOpen(false)}>
          DPM<span className="text-[var(--color-highlight)]">.</span>
        </a>
        ...
        <button type="button" className="p-2 md:hidden" aria-label="Menu" aria-expanded={open} onClick={() => setOpen(!open)}>
      ...
      {open && (
        <div className="border-t border-[var(--color-border)] bg-[var(--color-surface)] md:hidden">
          <ul className="section-shell flex flex-col gap-1 py-4">
            {navLinks.map((link) => (
              <li key={link.href}>
                <a href={link.href} className="block rounded-lg px-3 py-2.5 hover:bg-[var(--color-surface-alt)]" onClick={() => setOpen(false)}>
                  {link.label}
                </a>
              </li>
            ))}
