...
function evenR(count, lo, hi, rand) {
...
function buildLayout(groups) {
  ...
  order.forEach((o, k) => {
    ...
    const place = (im, ang, desired) => {
      ang += (Math.random() - 0.5) * (Math.PI / 9);
      desired *= 1 + (Math.random() - 0.5) * 0.4;
...
function ConstellationStar({ node, index, onActive }) {
  const { im, color } = node;
